...
while getopts hv:m:p:f:? option; do
  case "${option}" in
    h) usage; exit 1;;
...
"${currentPath}/build.sh" -v "${version}" -d "${deployId}"
"${currentPath}/link.sh" -d "${deployId}"
"${currentPath}/../ops/maintenance-start.sh"
"${currentPath}/switch.sh" -d "${deployId}"
"${currentPath}/../ops/cache-clean.sh"
"${currentPath}/prepare.sh" -d "${deployId}"
"${currentPath}/generate-before.sh"
if [[ -n "${memoryLimit}" ]]; then
  "${currentPath}/../ops/database-upgrade.sh" -m "${memoryLimit}"
else
  "${currentPath}/../ops/database-upgrade.sh"
fi
"${currentPath}/../ops/cache-clean.sh"
if [[ -n "${memoryLimit}" ]]; then
  "${currentPath}/../ops/generate.sh" -m "${memoryLimit}"
else
...
"${currentPath}/deploy-date.sh" -d "$(LC_ALL=en_US.utf8 date +"%Y-%m-%d %H:%M:%S %z")"
